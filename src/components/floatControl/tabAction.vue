<template>
  <q-btn-toggle class="toogleClass" style="margin: 10px; place-self: center" no-caps clearable color="white"
    text-color="#666666" toggle-color="secondary" v-model="buttonModel" :options="options"
    @update:model-value="selectControl" @clear="clearControl">
    <template v-slot:one>
      <q-badge v-if="lineStringCount > 0" color="yellow" text-color="white" floating>
        {{ lineStringCount }}
      </q-badge>
    ...
    <template v-slot:two>
      ...
      <q-badge v-if="polygonCount > 0" color="yellow" text-color="white" floating dense>
        {{ polygonCount }}
      </q-badge>
    </template>
  ...
  <q-list overlay v-if="drawList.length > 0">
    <q-scroll-area class="drawListClass" :style="`height: ${drawList.length * 51}px;`">
      <q-item v-for="(item, index) of drawList" :key="index" clickable @click="zoomToDraw(item.position)">
    ...
    <q-separator />
    <q-btn flat dense color="grey" icon="delete" style="float: right" @click="deleteDraw()" />
  </q-list>
</template>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const snap = ref(null);
    const drawList = ref([]);
    if (localStorage.getItem("drawList")) drawList.value = JSON.parse(localStorage.getItem("drawList"))
    ...
    const addInteraction = (type) => {
      ...
      drawEnd.value = unref(draw).on("drawend", function (evt) {
        ...
        localStorage.setItem("drawList", JSON.stringify(unref(drawList)));
      });
      ...
      if (index !== -1) {
        ...
        for (let i = 0; i < layers.length; i++) {
          ...
          if (layer instanceof VectorLayer) {
            ...
            const feature = source.getFeatureById(unref(drawList)[index].id);
